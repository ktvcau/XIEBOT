...
async function checkAppstate(APPSTATE_PATH, APPSTATE_PROTECTION) {
    ...
    if (!isExists(APPSTATE_PATH, "file")) {
        ...
        logger.custom(getLang("modules.checkAppstate.foundAppstate"), "CHECK");
    }
    ...
    if (APPSTATE_PROTECTION !== true) {
        logger.custom(getLang("modules.checkAppstate.noProtection"), "CHECK");
...
async function getAppStateNoProtection(
    ...
    try {
        if (isJSON(appState)) {
            ...
            logger.custom(getLang("modules.checkAppstate.decrypting"), "CHECK");
